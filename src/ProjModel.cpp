...
// 生成水滴网格的顶点和索引
bool Drops2Model::generate_mesh_(const float max_size_vert, std::vector<float>& vertices, std::vector<uint>& indices)
{
    // 参数校验
    if (fabs(max_height) <= epsilon || fabs(rad_at_base) <= epsilon)
        return false;
...
// 生成水滴网格的索引，索引决定了如何将顶点组合成三角形以构成网格
void Drops2Model::generate_indices_(std::vector<uint>& indices, const uint grid_size, const uint idx_min_y, const int32 last_vert) 
{
    bool oddRow = false;  // 判断当前行是奇数行还是偶数行
